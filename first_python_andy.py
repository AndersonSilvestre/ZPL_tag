        ...
        if desc != "":
            ...
            if quantEtiquetas.get() < 1:
                ...
                if response.status_code == 200:
                    ...
                    with open(codInt.get()+'.pdf', 'wb') as out_file: # change file name for PNG images
                        shutil.copyfileobj(response.raw, out_file)
                    messagebox.showinfo("PDF","PDF Gerado")                        
                    aksbox = messagebox.askquestion("nova","Gostaria de Gerar mais etiquetas?")
                    if aksbox == 'yes':
                        pass
                    else:
                        print("negou")
...
#buttons
btnQuit = ttk.Button(app, text="Sair", command=app.destroy).grid(column=1, row=3)
btnPdf = ttk.Button(app, text="Gera Etiquetas", command=geraPdf).grid(column=0, row=3)
#Button(app, text="show", command=callbackFunc).grid(column=0, row=3)
